...
public class BuyEquipCommand : AbstractCommand {
    ...
    protected override void OnExecute() {
        ...
        if (this.GetModel<IUserModel>().Star.Value > mEquipInfo.star) {
            this.SendCommand<ShowInfoConfirmAlertCommand>(new ShowInfoConfirmAlertCommand(content: string.Format(this.GetSystem<II18NSystem>().GetText("Whether to spend {0} star on this equip"),
                mEquipInfo.star),
            success: () => {
