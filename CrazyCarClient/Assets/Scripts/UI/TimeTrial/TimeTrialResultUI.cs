...
public class TimeTrialResultUI : MonoBehaviour, IController {
    ...
    private async void UpdateUI() {
        ...
        completeTimeSlider.value = this.GetModel<ITimeTrialModel>().IsComplete
            ? ((float)this.GetModel<ITimeTrialModel>().CompleteTime /
               this.GetModel<ITimeTrialModel>().SelectInfo.Value.limitTime)
            : 0;
        rewardText.text = this.GetModel<ITimeTrialModel>().RewardStar.ToString();
        rankText.text = this.GetModel<ITimeTrialModel>().IsBreakRecord
            ? this.GetModel<ITimeTrialModel>().Rank.ToString()
            : "--";
    ...
    private void Start() {
        confirmBtn.onClick.AddListener(() => { this.SendCommand(new LoadSceneCommand(SceneID.Index)); });

        rankBtn.onClick.AddListener(() => { timeTrialRankUI.gameObject.SetActiveFast(true); });
    }
