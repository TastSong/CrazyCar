...
public class AIController : MonoBehaviour, IController {
    ...
    private async void OnMakeAIPlayer(MakeAIPlayerEvent e) {
        ...
        Util.DelayExecuteWithSecond(aiInfo.startMoveTime, () => { aiInfoDic[aiInfo.id].isStartGame = true; });
    }
        ...
        foreach (var item in aiInfoDic) {
            if (item.Value.isStartGame) {
                ...
                if (item.Value.needPassTimes == item.Value.mPlayer.passEndSignTimes) {
                    ...
                    item.Value.mPlayer.transform.position =
                        item.Value.pathCreator.path.GetPointAtDistance(item.Value.distanceTravelled);
                    item.Value.mPlayer.transform.rotation = Quaternion.Euler(
                        item.Value.pathCreator.path.GetRotationAtDistance(item.Value.distanceTravelled).eulerAngles +
